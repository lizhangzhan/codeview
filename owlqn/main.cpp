...
void printUsageAndExit() {
	...
	cout << "  -sls           use sparse least squares" << endl;
...
int main(int argc, char* argv[]) {
	...
	bool leastSquares = false, sparseLeastSquares=false, quiet = false;
	double tol = 1e-4, l2weight = 0;
	int m = 10;

	for (int i=5; i<argc; i++) {
		if (!strcmp(argv[i], "-ls")) leastSquares = true;
		else if (!strcmp(argv[i], "-sls")) sparseLeastSquares = true;
	...
	else if (sparseLeastSquares) {
		SparseLeastSquaresProblem *prob = new SparseLeastSquaresProblem(feature_file);
		obj = new SparseLeastSquaresObjective(*prob, l2weight);
		size = prob->NumFeats();
	}
	else {
